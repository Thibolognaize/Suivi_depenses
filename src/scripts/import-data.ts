...
async function cleanDatabase() {
    ...
    try {
        ...
        await prisma.$executeRaw`ALTER TABLE collaborateur AUTO_INCREMENT = 1;`
...
async function importData() {
    try {
        // Vérifiez si le script de seed a déjà été exécuté
        const seedRecord = await prisma.seed.findFirst()

        if (seedRecord) {
            console.log('Script import-data already executed. Skipping...')
            return
        } else {
            await cleanDatabase()
        }
        ...
        // Marquez le script de seed comme exécuté
